...
export function SocialLinks({ visible, className }: SocialLinksProps) {
  return (
    <div
      className={cn(
        "mt-4 flex",
        visible && "animate-hero-rise",
        !visible && "opacity-0",
        className
      )}
    >
      {socialLinks.map((link) => {
        const Icon = iconMap[link.icon as keyof typeof iconMap] ?? GitHubIcon;

        return (
          <MagneticWrap key={link.href} className="mr-3">
            <a
              href={link.href}
